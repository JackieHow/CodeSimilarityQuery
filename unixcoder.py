...
class UniXcoder(nn.Module):
    def __init__(self, model_name, local_model_path='~/.cache/huggingface/transformers'):
        ...
        self.local_model_path = os.path.expanduser(local_model_path)
    ...
    def load_tokenizer(self):
        tokenizer_path = os.path.join(self.local_model_path, 'tokenizer')
    ...
    def load_model(self):
        model_path = os.path.join(self.local_model_path, 'pytorch_model.bin')
        if not os.path.exists(model_path):
            model = RobertaModel.from_pretrained(self.model_name, config=self.config)
            model.save_pretrained(self.local_model_path)
        else:
            model = RobertaModel.from_pretrained(self.local_model_path)
        return model
